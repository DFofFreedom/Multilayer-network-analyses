...
#load data
load("adult_inters_mx.RData") #load basic adult interaction data, symmetrical list of edgelists, and data formatted into muxViz style
#all generated by "Cuddling inters to muxviz Functions for Github" code

#Reducibility analysis####
source("C:/Users/s02df9/OneDrive/Current Projects/R analysis/ML nets/muxViz-master//muxLib DF.R") #set source to the location of the "muxLib DF.R" file
...
adult_inters_SAM = lapply(adult_inters_mx, build_supra_AM)

#Colony temporal reducibility to attack speed####

adult_inters_reduce = list()
...
lapply(adult_inters_reduce, '[[', "gQualityFunction")
#10 and 24 show peaks not at max layers.

plot_ML_reduce = function(reduce_list) {
  points(1:19,reduce_list$gQualityFunction, type="l")}

plot_ML_reduce2 = function(reduce_list) {
  points(1:19,reduce_list$gQualityFunction, type="l", lty=2)}

plot(1, type="n", xlab="Amount of aggregation", ylab="Difference in relative entropy",
...
col_reduce_info = unlist(lapply(adult_inters_reduce,'[[',"gQualityFunction")) 

col_info_data = as.data.frame(cbind(col_reduce_info, 
                                    rep(unique(adult_inters$Colony),
                                        each=length(unique(adult_inters$layer1))),
                                    1:length(unique(adult_inters$layer1))))
colnames(col_info_data) = c("information", "colony", "layer")
col_info_data$colony = factor(col_info_data$colony)
...
#Download from figshare, I checked and this matched exactly the csv file NPW sent
...
names(adult_prey)[19] = "latency_to_attack" #correct typo
...
with(attack_to_reduce_data, plot(max_info, m_latency_att, pch=16, frame=F,ylim=c(0,500),xlim=c(5,15),
...
with(attack_to_reduce_data, plot(max_info, cv_latency_att, pch=16,frame=F,ylim=c(0,1.5),xlim=c(5,15),
                                 xlab="Variability of network", ylab="Coefficient of variation\nin latency to attack",
                                 las=1))
text(2,1.8,"b",font=2)
box(which = "plot", bty = "l")
...
#Plot the distributions of the randomisaed correlation coefs for the supp mat
