...
download.file("https://ndownloader.figshare.com/files/21743832", 
              destfile = here("data", "adult_inters_raw.csv"))
...
adult_inters = adult_inters_raw %>% 
  mutate(node1 = factor(paste0(adult_inters_raw$Spider.1,"_",adult_inters_raw$Colony)),
         ...
         layer2  = layer1,
         weight = 1)
head(adult_inters) # structured as dyads--each row is a pair of spiders.
...
plot(with(adult_inters, tapply(node1, layer1, length)) ~ c(1:19),
     ...
     ylab="Number of associations", xlab="Time point",main="") # associations generally decline over successive time points
...
#Edgelists are not symmetrical (i.e. they only contain one listing of each dyad, not two), so need to copy data and then swap node1 and node2
...
all(unlist(lapply(adult_inters_col, nrow))*2 == unlist(lapply(adult_inters_sym, nrow))) # TRUE
...
#not symmetrising as in above code, as these edges are not within layers
